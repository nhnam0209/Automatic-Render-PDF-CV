    ...
    for _item in data["list_section"]:
        ...
        if _item["name"] == "education" and _item["data"] != "" and _item["status"] == "on":
            if _item["data"][0]["title"] != "":
                education += "%===============================EDUCATION========================\n"
                education += "\\resheading{Education}\n\\begin{itemize}\n\n"
                for item in _item["data"]:
                    if item["degree"] == "":
                        item["degree"] = " "
                    if item["time"] == "":
                    ...
                    if item["gpa"] == "":
                        item["gpa"] = " "
                    education += "\\item \\ressubheading" \
                                     "{" + str(item["title"]) + "}{" + str(item["org"]) + "}{" + str(
                            item["degree"]) + "}{" + str(item["time"]) + \
                                     "}{GPA: " + str(item["gpa"]) + "}\n\n"
                    if len(item["desc"]) > 0:
                        if item["desc"][0] != "":
                            desc = item["desc"][0].strip("\n").split("\n")
                            education += "{\\bf{Awards/Certifications/Professional Development: }}"
                            education += "\\begin{itemize}\n"
                            for sub_item in desc:
                                education += "\t\\resitem{" + str(sub_item) + "}\n"
            ...
            if _item["data"][0]["title"] != "":
                award += "%===============================AWARD========================\n"
                award += "\\resheading{Awards}\n\\begin{enumerate}"
                for item in _item["data"]:
                    if item["time"] == "":
                        item["time"] = " "
                    if item["org"] == "":
                        item["org"] = " "
                    award += "\n\t\\item {" + str(item["title"]) + " \\hfill " + str(item["time"]) + "}\\\\{" + str(
                            item["org"]) + "}"
                award += "\n\\end{enumerate}\n"
            ...
            if _item["data"][0]["company"] != "":
                ...
                experience += "\\resheading{Experience}\n\\begin{itemize}\n\n"
                for item in _item["data"]:
                    if item["pos"] == "":
                        item["pos"] = " "
                    if item["time"] == "":
                    ...
                    if item["loc"] == "":
                        item["loc"] = " "
                    experience += "\\item \\ressubheading" \
                                      "{" + str(item["company"]) + "}{\\bf{" + str(item["pos"]) + "}}{" + str(
                            item["loc"]) + "}{" + str(item["time"]) + \
                                      "}\\\\\n\n"
                    if len(item["desc"]) > 0:
                        if item["desc"][0] != "":
                            desc = item["desc"][0].strip("\n").split("\n")
                            experience += "{\\bf{Short Description: }}"
                            experience += "\\begin{itemize}\n"
                            for sub_item in desc:
                                experience += "\t\\resitem{" + str(sub_item) + "}\n"
            ...
            if _item["data"][0]["title"] != "":
                project += "%===============================PROJECT========================\n"
                project += "\\resheading{Projects}\n\\begin{enumerate}"
                for item in _item["data"]:
                    if item["time"] == "":
                        item["time"] = " "
                    if item["desc"] == "":
                        item["desc"] = " "
                    project += "\n\t\\item " + str(item["title"]) + "\\hfill" + str(item["time"])
                    if len(item["desc"]) > 0:
                        if item["desc"][0] != "":
                            desc = item["desc"][0].strip("\n").split("\n")
                            project += "\n\t\\begin{itemize}"
