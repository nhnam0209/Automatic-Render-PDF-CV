...
def fill(data,media_path=None):
    ...
    name, email, phone, contact, personal_info, href_home,github, linkedin, birthdate,link, person  = tuple("" for i in range(11))
        ...
        if _item["name"] == "skill" and _item["data"] != "" and _item["status"] == "on":
            ...
            if _item["data"][0]["title"] != "":
                ...
                for item in _item["data"]:
                    ...
                    if item["gpa"] == "":
                        item["gpa"] = " "
                    education += "\\sepspace\n\\EducationEntry{"  + str(item["title"]) + "}{" + str(item["time"]) + "}\n\t\t\t{" +\
            ...
            if _item["data"][0]["company"] != "":
                ...
                for item in _item["data"]:
                    ...
                    if item["loc"] == "":
                        item["loc"] = " "
                    experience += "\\sepspace\n\\EducationEntry{" + str(item["pos"]) + "}{" + str(item["time"]) +"}{" + str(item["company"]) + "}{"
                    if len(item["desc"]) > 0:
                        if item["desc"][0] != "":
                            desc = item["desc"][0].strip("\n").split("\n")
                            experience += "\n\t\\begin{itemize}"
